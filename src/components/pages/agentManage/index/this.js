define(["ui/loading","common/util","activity","ui/header","ejs"],function(loading,util,Activity,header){
	var MyActivity = Activity.extend({
		init: function (options){
			...
			header.init(this.path,{title:'我的信息',goBack:false});
			this.reName();
		    this.initEvents();
		},
		reName:function(){
		...
		initEvents:function(){
			var elBox=$('.agentManage_index');
			elBox.on('click','.subordinate_management',function () {
				LWH.history.navigate('subordinate_management',true);
				header.setTitle()
			});
			elBox.on('click','.setting',function () {
				LWH.history.navigate('set',true);
