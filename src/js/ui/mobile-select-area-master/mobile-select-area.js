import '@/js/ui/mobile-select-area-master/dialog.js'

(function(root, factory) {
	//amd
	if (typeof define === 'function' && define.amd) {
		define(["jquery", '/js/ui/mobile-select-area-master/dialog'], factory);
	}else if (typeof define === 'function' && define.cmd) {
		define(function(require, exports, module) {
			var $ = require("lib/jquery/1.11.2/jquery.min");
	...
	MobileSelectArea.prototype = {
		init: function(settings) {
			...
			this.text = this.settings.text ? this.settings.text: this.trigger.val() ? this.trigger.val().split(' ') : ['', '', ''];
		...
		f: function(data) {
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0" && this.default == 0) {
				...
				if (item.length > 0 && this.default == 1) {
					...
					var id = item[0].id || 0;
					focus = item[0].id;
					childData = item[0].cityList  || item[0].areaList;
				...
				for (var j = _this.default, len = item.length; j < len; j++) {
					...
					if (_this.value[_this.index] == id) {
						cls = "focus";
						focus = id;
						childData = item[j].cityList || item[j].areaList;
