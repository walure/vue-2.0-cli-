(function(root, factory) {
	...
	$.Dialog = function(settings) {
		...
		if (settings.type === "confirm") {
			...
			for (var i = 0, l = settings.buttons.length; i < l; i++) {
				...
				if (item.yes) {
					btnstr += '<td><button class="ui-confirm-submit red" data-type="yes">' + item.yes + '</button></td>';
				}
				if (item.no) {
			...
			var options = $.extend({
				...
				beforeShow: function(c) {
					dialog.touch($('.ui-confirm-submit', c), function() {
						if($('.ui-confirm-submit').hasClass('green')){
	...
	Dialog.prototype = {
		init: function(settings) {
			...
			this.wrapper.append('<div class="ui-dialog" id="' + this.id + '"></div>');
			...
			if (this.settings.animate) {
				...
				setTimeout(function() {
					...
					if (typeof _this.settings.target === "object") {
						this.wrapper.append(_this.dialogContainer.hide());
					}
				...
				if (typeof this.settings.target === "object") {
					this.wrapper.append(this.dialogContainer)
				}
		...
		setPosition: function() {
			...
			//	console.log("ch:"+clientHeight,"cw:"+clientWidth,"left:"+left,"top:"+top,"w:"+this.width,"h:"+this.height);
				...
				/* 添加green 可选择*/
				if($('.ui-scroller-mask dl').length>2){
					if($('.ui-scroller-mask dl').eq(0).css('top').split('px')[0]<=0 && $('.ui-scroller-mask dl').eq(1).css('top').split('px')[0]<=0 && $('.ui-scroller-mask dl').eq(2).css('top').split('px')[0]<=0){
						if(!$('.ui-confirm-submit').hasClass('green')) $('.ui-confirm-submit').addClass('green');
					}else{
						$('.ui-confirm-submit').removeClass('green');
					}
				}else{
					$('.ui-confirm-submit').removeClass('green');
				}
